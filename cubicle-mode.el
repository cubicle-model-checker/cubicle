...
(defvar my-meta-face 'my-meta-face)

(defvar cubicle-font-lock-keywords
  '(
    ("(\\*\\([^*]\\|\\*[^)]\\)*\\*)" . font-lock-comment-face)
    ; transitions need not have a return type
    ("\\(meta_transition\\)\\s-+\\(\\sw+\\)" (1 my-meta-face) (2 font-lock-function-name-face))
    ("\\(univ_transition\\)\\s-+\\(\\sw+\\)" (1 font-lock-keyword-face) (2 font-lock-function-name-face))
...
(define-derived-mode cubicle-mode fundamental-mode "Cubicle"
  ...
  (when (buffer-file-name)
    (set (make-local-variable 'compile-command)
         (format "../cubicle.opt %s" (file-name-nondirectory buffer-file-name))))
