...
def job(test_file: Path) -> tuple[dict, dict]:
    arith_list, base_list = [], []

    with ThreadPoolExecutor(max_workers=2) as executor:
        future_to_file = {executor.submit(run_cubicle, cubicle_opt, test_file): "Arith",
                          executor.submit(run_cubicle, cubicle, test_file): "Base"}

        for future in as_completed(future_to_file):
            type_tested = future_to_file[future]
            cub_result = future.result()
            if type_tested == "Arith":
                arith_list.append(cub_result)
            elif type_tested == "Base":
                base_list.append(cub_result)
            else:
                raise RuntimeError("Who the fuck are you ?")

    assert (len(arith_list) == 1 == len(base_list))
    return arith_list[0], base_list[0]


def run_all(nb_workers: int) -> dict[str, tuple[dict, dict]]:
    testing_data = {}
    with ThreadPoolExecutor(max_workers=nb_workers) as executor:
        future_to_file = {executor.submit(job, file): file for file in examples_list}

        for future in as_completed(future_to_file):
            tested_file = future_to_file[future]
            cub_result = future.result()
            testing_data[tested_file.stem] = cub_result

    return testing_data


def dict_to_list(file: str, run_data: tuple[dict, dict]) -> list[str]:
    arith, base = run_data
...
def save_to_csv(data_to_save: dict[str, tuple[dict, dict]]) -> None:
    if OUT_FILE.exists():
        OUT_FILE.unlink()
    ...
    with open(OUT_FILE, "w") as f:
        f.write(";".join(legend) + "\n")
        for file, d in data_to_save.items():
            f.write(";".join(dict_to_list(file, d)) + "\n")


if __name__ == '__main__':
    data = run_all(NB_PROCESS // 2)
    save_to_csv(data)
