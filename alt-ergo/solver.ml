...
let var_bump_activity v = 
  ...
  if Heap.in_heap env.order v.vid then begin
    if false || D1.d_decisions then eprintf "decrease %d with new weight %f@." 
      (v.vid+1) v.weight;
...
let analyze c_clause = 
  if D1.d then eprintf "conflicting clause: %a@." D1.clause c_clause;
  let pathC  = ref 0 in
  let learnt = ref [] in
...
let simplify () = 
  ...
  if nb_assigns() <> env.simpDB_assigns && env.simpDB_props <= 0 then begin
    ...
    (*Heap.filter env.order f_filter f_weight;*)
...
let record_learnt_clause blevel learnt size = 
  D1.analyze_conflict blevel learnt;
...
let theory_analyze dep = 
  if D1.d then eprintf "explication: %a@." Ex.print dep;
  let atoms = ref []in
...
let search n_of_conflicts n_of_learnts = 
  ...
  while (true) do
    match propagate () with
      | Some confl -> (* Conflict *)
          incr conflictC;
	  D1.boolean_conflict (decision_level()) conflictC; 
	  ...
	  match theory_propagate () with
		...
		incr conflictC;
		env.conflicts <- env.conflicts + 1;
		D1.theory_conflict (decision_level()) conflictC; 
		...
		D1.decide env.decisions current_level next.pa;
		assert (next.level < 0);
		enqueue next.pa current_level None
  done

let check_clause c = 
  if D1.d then eprintf "check_clause:@.";
  let b = ref false in
  let atoms = c.atoms in
  if D1.d then eprintf " { ";
  for i = 0 to Vec.size atoms - 1 do
    let a = Vec.get atoms i in
    if D1.d then eprintf "%a ; " D1.atom a;
    b := !b || a.is_true
  done;
  if D1.d then eprintf "}@.";
  assert (!b)
...
let solve () = 
  ...
  if D1.d_decisions then begin
    ...
    eprintf "| Conflicts |          ORIGINAL         |          LEARNT          | Progress |@.";
    eprintf "|           |    Vars  Clauses Literals |    Limit  Clauses Lit/Cl |          |@.";
    eprintf "===============================================================================@.";
  end;
  try while true do
    if D1.d_decisions then begin
      eprintf "| %9d | %7d %8d %8d | %8d %8d %6.0f | %6.3f %% |@."
        env.conflicts
        (Heap.size env.order)
        (nb_clauses()) 
	...
	| a::_::_ -> 
	    ...
	    if a.neg.is_true then begin
	      ...
	      add_boolean_conflict clause
            ...
            D1.add_clause clause;
            (*D1.watched_of_a (Vec.get clause.atoms 0).neg;
              D1.watched_of_a (Vec.get clause.atoms 1).neg;*)
            
	| [a]   ->
	    cancel_until 0;
	    (* if a.var.level > 0 then *)
	    (*   (\* backtrack to level before level of a *\) *)
	    (*   cancel_until (a.var.level - 1); *)

            D1.add_unit_clause a;
            enqueue a 0 None;
...
let init_solver cnf =
  ...
  if Debug1.d_decisions then begin
    ...
    eprintf "|  Number of clauses:    %-12d                                         |@." nbc;
  end;
  Vec.grow_to_by_double env.vars nbv;
  Heap.grow_to_by_double env.order nbv;
  List.iter 
    (List.iter 
       (fun a ->
