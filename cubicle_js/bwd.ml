...
let loop_res = ref None
let loop_done = ref false
...
module Make ( Q : PriorityNodeQueue ) : Strategy = struct
  ...
  let search ?(invariants=[]) ?(candidates=[]) system =
    ...
    (* let after res =  *)
    (*   match res with  *)
    (*     |None -> Safe (Cubetrie.all_vals !visited, !candidates) *)
    (*     |Some s -> s  *)
    (* in  *)
      (* try *)
    ...
    let rec loop () =  
      try 
        if not (Q.is_empty q) then
          ...
          (Firebug.console##log (Js.string "bwd fini");
           loop_res := Some (Safe (Cubetrie.all_vals !visited, !candidates));
           loop_done := true;)
      with Safety.Unsafe faulty -> 
        (loop_res_i := Some (faulty);
         loop_res := Some (Unsafe (faulty, !candidates));
         loop_done := true)
      (* done; *) in 
      loop ();
      match !loop_res_i with 
        |None -> (Safe (Cubetrie.all_vals !visited, !candidates))
        |Some f -> (Unsafe (f, !candidates)) 
(* with Safety.Unsafe faulty -> *)
