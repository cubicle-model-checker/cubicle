  ...
  fun sa ->
    ...
    let rec aux ((tosimpl, nomod) as acc)  = function
      | Atom.Comp (v1, Neq, (Elem (h, _))) :: tl ->
        let tosimpl = TM.update v1 (function
            | None -> Some (HSet.singleton h)
            | Some hs -> Some (HSet.add h hs)
          ) tosimpl in
        aux (tosimpl, nomod) tl
      | a :: tl -> aux (tosimpl, SAtom.add a nomod) tl
      | _ -> acc
    in
    let tm, nomod = aux (TM.empty, SAtom.empty) l in
    TM.fold (fun t hs (simpl, nomod) ->
        let d = HSet.diff sfp hs in
        if HSet.cardinal d = 1 then
          SAtom.add (Atom.Comp (t, Eq, Elem (HSet.min_elt d, Var))) simpl, nomod
        else
          let sa = SAtom.filter (function
              | Atom.Comp (t', _, _) -> Term.equal t t'
              | _ -> false) sa in
          simpl, SAtom.union sa nomod
      ) tm (SAtom.empty, nomod)
...
let create_norma_sa_ar sa ar =
  ...
  if test_size_proc vars then
    ...
    let new_ar = ArrayAtom.apply_subst sigma ar in
    let new_sa = ArrayAtom.to_satom new_ar in
    let new_sa = if !size_proc <> 0 then
        let gsa = SAtom.glob_atoms new_sa in
        let simpl, nomod = simplify_satom gsa in
        let sa = SAtom.diff new_sa gsa in
        SAtom.union simpl (SAtom.union nomod sa)
      else new_sa in
    {
      vars = new_vars;
      litterals = new_sa;
