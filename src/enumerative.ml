...
let state_to_cube env st =
  ...
  Array.fold_left (fun sa sti ->
	...
	let t2 =
          if sti = env.minf_int_abstr then Term.Elem (Hstring.make "-oo", Constr)
          else if sti = env.pinf_int_abstr then Term.Elem (Hstring.make "+oo", Constr) 
...
let is_int_real = function
  | Term.Elem (x,Glob) | Term.Access (x, _) -> 
    snd (Smt.Symbol.type_of x) = Smt.Type.type_int ||
  snd (Smt.Symbol.type_of x) = Smt.Type.type_real
  | _ -> false

let all_constr_terms () =
  List.rev_map (fun x -> Term.Elem (x, Constr)) (Smt.Type.all_constructors ())

let terms_of_procs = List.map (fun x -> Term.Elem (x, Var))
...
let add_pos_to_proc_substate ht
  ...
  Term.Set.iter (function
    | Term.Access (_, ps) as t ->
      let i = HT.find ht t in
      let ids_ps = List.map (fun hp -> HT.find ht (Term.Elem (hp, Var))) ps in
  ...
  let map_orders =
    Term.Set.fold (fun t acc -> match t with
      | Term.Access (a, ps) ->
        ...
        let t_par = Term.Access (a, ups) in
...
let init_tables ?(alloc=true) procs s =
  ...
  let id_true =
    try HT.find ht (Term.Elem (Term.htrue, Constr)) with Not_found -> -2 in
  let id_false =
    try HT.find ht (Term.Elem (Term.hfalse, Constr)) with Not_found -> -2 in
  
  let a_low = !i in
  List.iter (fun c ->
    HT.add ht (Term.Const (MConst.add (ConstInt (Num.Int c)) 1 MConst.empty)) !i;
    HT.add ht (Term.Const (MConst.add (ConstReal (Num.Int c)) 1 MConst.empty)) !i;
    incr i) abstr_range;
  let a_up = !i - 1 in
...
let abs_inf = 
  SAtom.filter (function
    | Atom.Comp ((Term.Elem (x, Glob) | Term.Access (x,_)), _, _) ->
...
let write_atom_to_states env sts = function
  | Atom.Comp (t1, (Le | Lt as op), (Term.Const _ as t2)) when abstr_num ->
    ...
    !l
  | Atom.Comp ((Term.Const _ as t1), (Le | Lt as op), t2) when abstr_num  ->
  ...
  | Atom.Comp (t1, Eq, t2) ->
    ...
    sts
  | Atom.Comp (t1, Neq, Term.Elem(_, Var)) ->
...
let is_general env l = 
  ...
  List.for_all (fun (i, _) ->
    ...
    match ti with
      | Term.Access (h, _) -> if HH.mem seen h then false else
          (HH.add seen h (); true)
      | _ -> false
  ) l
...
let generalize_state env s hs system =
  ...
  let vars = hs in
  List.iter (fun arr ->
    let ta = Term.Access(arr, vars) in
    try
