...
let rec mem cube trie = match trie with 
  | Empty -> None
  | Full vl -> Some (List.map (fun v -> v.tag) vl) (* Should also check trace *)
  | Node l -> match cube with
      | [] -> None
      | atom::cube -> 
...
let rec mem_poly cube trie = match trie with 
  | Empty -> false
  | Full _ -> true (* Should also check trace *)
  | Node l -> match cube with
      | [] -> false
      | atom::cube -> 
...
let rec consistent cube trie = match cube, trie with
  | [], _ -> all_vals trie
  | _, Empty -> []
  | _, Full vl -> vl (* Should check memory trace *)
