...
/*
cons_arg:
  | mident OF lident { [($1,$3)] }
  | mident OF lident BAR cons_arg { ($1,$3)::$5 }
;*/
...
constructors:
  | mident { [($1, None)] }
  | mident BAR constructors { ($1,None)::$3 }
  | mident OF lident { [($1, Some $3)] }
  | mident OF lident BAR constructors { ($1, Some $3)::$5 }
;*/
...
macros:
  | MACRO lident LEFTBR expr RIGHTBR {assert false }
  | MACRO lident LEFTPAR lidents RIGHTPAR LEFTBR expr RIGHTBR {assert false}
...
update:
  ...
  | mident /*LEFTSQ proc_name_list_plus RIGHTSQ*/ AFFECT mident LEFTSQ proc_name COLON proc_name RIGHTSQ
      {
      (*i = $8, j = $10 --> 7*)
	(* M[k] := N[i:j] -->
	   M[k] := case | i < k < j : N[k]
	...
	*)
	...
	if List.length $3 <> 1 then raise Parsing.Parse_error;*)
	let k = fresh_var() in(*List.hd $3 in*)
	let ik = ALe(TVar $5, TVar k) in
	let kj = ALt (TVar k, TVar $7) in
	let conj = PAnd[PAtom ik; PAtom kj] in 
	let mk = Access($3, [k]) in
	let mk1 = Access($1, [k]) in 
	let other = [(PAtom(AAtom(Atom.True)), TTerm mk1)] in
	let f = (conj, TTerm mk)::other in
	Upd { pup_loc = loc (); pup_arr = $1; pup_arg = [k]; pup_swts = f }
	
      }

  | mident AFFECT mident LEFTSQ proc_name COLON proc_name RIGHTSQ AT mident LEFTSQ proc_name COLON proc_name RIGHTSQ
      {
	(*
	  X := A[i:j]@B[l:m]
	  --> 
	...
	*)
	...
	let p = fresh_var () in
	let c1_ip = ALe(TVar $5, TVar p) in
	let c1_pj = ALt(TVar p, TVar $7) in
	let conj_c1 = PAnd[PAtom c1_ip; PAtom c1_pj] in
	let c2_lp = ALe(TVar $12, TVar p) in
	let c2_pm = ALt(TVar p, TVar $14) in
	let conj_c2 = PAnd [PAtom c2_lp; PAtom c2_pm] in
	let access = Access($1, [p]) in
	let c1 = Access($3, [p]) in
	let c2 = Access($10, [p]) in

	let other =  [(PAtom(AAtom(Atom.True)), TTerm access)] in
	let f = (conj_c1, TTerm c1)::(conj_c2, TTerm c2)::other in
	Upd { pup_loc = loc (); pup_arr = $1; pup_arg = [p]; pup_swts = f }
...
/*
update_array:
  | mident LEFTSQ INT RIGHTSQ AFFECT term { assert false}
;*/
 ...
 */
  ...
  | mident LEFTSQ INT RIGHTSQ { BitVAccess ($1, $3) }
  | LEFTSQ BAR term_list BAR  RIGHTSQ { BitVector $3 } 
  /*| array_extr {$1}*/
/*| concat_list {BitVConcat $1}*/
  | array_op {BitVOp $1}

;

array_extr:
  | mident LEFTSQ INT COLON INT RIGHTSQ {($1, ($3, $5))} 
;

array_op:
  | array_extr { [$1] }
  | array_extr AT array_op {$1::$3}
;
...
let a, b = $1, $3 in
    ...
    | term LT term { Smt.set_arith true;
		     let a, b = $1, $3 in
      let el = 
      match a with
	| Elem(v1, Var) -> TVar v1
...
(*ALt (TTerm $1, TTerm $3)*) }
    | term LE term { Smt.set_arith true;
		     let a, b = $1, $3 in
      let el = 
      match a with
	| Elem(v1, Var) -> TVar v1
...
(*ALe (TTerm $1, TTerm $3)*) }
    | term GT term { Smt.set_arith true;
		     let a, b = $1, $3 in
		     let el = 
		       match a with
			 | Elem(v1, Var) -> TVar v1
			...
			ALt (el, el2)
		     ...
		     (*ALt (TTerm $3, TTerm $1)*) }
    | term GE term { Smt.set_arith true;
		     let a, b = $1, $3 in
      let el = 
      match a with
	| Elem(v1, Var) -> TVar v1
