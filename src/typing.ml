...
let create_node_universal nbp kind evars uvar sa =
  ...
  let unsa, rest = SAtom.partition (has_var uvar) sa in
  let nvsa, exsa = SAtom.partition has_no_vars rest in
  ...
  let rec aux (iexsa, ievars, iunsa) evars rem_vars = match evars, rem_vars with
    | [], _ -> (iexsa, ievars, iunsa), rem_vars
    | hd1 :: tl1, hd2 :: tl2 ->
      let s = build_subst [hd1] [hd2] in
      (* eprintf "subst : %a@." print_subst s; *)
      let iexsa = SAtom.subst s iexsa in
      let iunsa = SAtom.subst s iunsa in
      aux (iexsa, hd2::ievars, iunsa) tl1 tl2
    | _ -> assert false
  in
  (* eprintf "Exist : @."; *)
  let (iexsa, ievars, iunsa), rem_vars =
    aux (SAtom.union exsa nvsa, [], unsa) evars inst_vars in
  (* Format.eprintf "IEXSA : %a@." SAtom.print iexsa; *)

  let isa = List.fold_left (fun acc p ->
    let s = build_subst [uvar] [p] in
    (* eprintf "subst : %a@." print_subst s; *)
    let acc = SAtom.union (SAtom.subst s iunsa) acc in
  ...
  let c = Cube.normal_form c in
  (* Format.eprintf "Cube N : %a@." Cube.print c; *)
  (* assert false; *)
