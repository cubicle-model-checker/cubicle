...
let print_term fmt = function
  | TVar (v, l) -> 
    if l.active then Hstring.print fmt v.hstr else fprintf fmt "(* %a *)" Hstring.print v.hstr
  | TTerm (t, l) -> 
    if l.active then Term.print fmt t else fprintf fmt "(* %a *)" Term.print t

let print_atom fmt = function
  | AVar (v, l) -> 
    if l.active then Hstring.print fmt v.hstr else 
      (fprintf fmt "(* %a *)" Hstring.print v.hstr)
  | AAtom (a, l) -> 
...
and
    print_f fmt  = function 
      ...
      | PNot (not_i,f,loc) -> 
        if not_i.active then 
          fprintf fmt "( not (%a) )" print_formula f
        else 
          fprintf fmt "( (*not*) %a )" print_formula f
      | PAnd (l,loc) -> fprintf fmt " %a " print_list_sep_form (print_formula, l, " && ")
      | POr (l,loc) -> fprintf fmt " %a " print_list_sep_form (print_formula, l, " || ")
      | PImp (a, b, loc) -> fprintf fmt "( %a => %a )" print_formula a print_formula b
      | PEquiv (a, b, loc) -> fprintf fmt "( %a <=> %a )" print_formula a print_formula b
      | PIte (c, t, e, loc) -> fprintf fmt " if (%a) then (%a) else (%a) " print_formula c print_formula t print_formula e
      | PForall (vs, f, loc) -> 
        (fprintf fmt " forall";
         List.iter (fun (x) -> fprintf fmt " %a " Variable.print x.hstr) vs;
...
let rec print_list_seph fmt (l, sep) = 
  match l with
    |[] -> ()
    |x::[] ->  fprintf fmt "%s" (Hstring.view x.hstr)
    |x::s -> fprintf fmt "%s%s%a" (Hstring.view x.hstr) sep print_list_seph (s,sep) 

let rec print_list_sep fmt (l, sep) =
  match l with
    |[] -> ()
    |x::[] ->
      if x.hstr_i.active then
...
let print_list_sep_type fmt (l, sep) =
  let rec print fmt = function
    |[] -> ()
    |x::[] -> if x.hstr_i.active then
        fprintf fmt "%s" (Hstring.view x.hstr) 
      else
        fprintf fmt "(*%s *)" (Hstring.view x.hstr) 
    |x::s ->
      if x.hstr_i.active  && not_active s  then
        fprintf fmt "%s%a" (Hstring.view x.hstr)  print s
      else if x.hstr_i.active then
...
let print_type_defs fmt l = 
  List.iter (fun (loc, (name, t_l)) -> 
    if loc.active then
      fprintf fmt "type %s = %a@." (Hstring.view name.hstr) print_list_sep_type (t_l, " | " ) 
    else
      fprintf fmt "(* type %s = %a*)@." (Hstring.view name.hstr) print_list_sep_type (t_l, " | " )) l
...
let print_consts_or_globals fmt (str, l) = 
  List.iter (fun (loc, name, t) -> 
    if loc.active then
      ...
      fprintf fmt "(* %s %s : %s*)@." str (Hstring.view name.hstr) (Hstring.view t.hstr)) l 
  ...
  List.iter (fun (loc, name, (t_l, t)) -> 
    if loc.active then 
      fprintf fmt "array %s [%a] : %s@." 
        (Hstring.view name.hstr) print_list_seph (t_l, ",") (Hstring.view t.hstr) 
    else
      fprintf fmt "(* array %s [%a] : %s*)@." 
        (Hstring.view name.hstr) print_list_seph (t_l, ",") (Hstring.view t.hstr)) l
...
let print_assigns fmt l =
  List.iter (fun x ->
    if x.a_i.active then
      ...
      fprintf fmt "(* %s := %a; *)@;" (Hstring.view x.a_n.hstr) print_glob_update x.a_p)l
    ...
    if u.pup_loc.active then
      (match u.pup_info with 
        |None -> let (x,_) = u.pup_swts in 
          fprintf fmt "%s[%a] := %a;@,"
            (Hstring.view u.pup_arr.hstr) print_list_seph (u.pup_arg,", ") print_swts x  
...
let print_transitions fmt l = 
  List.iter (fun t ->
    if t.ptr_loc.active then
      fprintf fmt "@[transition %s (%a)@.requires { @[%a@] }@;{%a@]@;}@.@."
        (Hstring.view t.ptr_name.hstr)  print_list_sep (t.ptr_args, " ") 
