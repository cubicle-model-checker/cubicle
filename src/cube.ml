...
let redondant_or_false others a = match a with
  | Atom.True -> Atom.True
  | Atom.Comp (t1, Neq, (Term.Elem (x, (Var | Constr)) as t2))
  | Atom.Comp ((Term.Elem (x, (Var | Constr)) as t2), Neq, t1) ->
    (try 
       (SAtom.iter (function 
	 | Atom.Comp (t1', Eq, (Term.Elem (x', (Var | Constr)) as t2'))
	 | Atom.Comp ((Term.Elem (x', (Var | Constr)) as t2'), Eq, t1') 
...
let rec simplify_term = function
  | Term.Arith (Term.Arith (x, c1), c2) ->
    simplify_term (Term.Arith (x, add_constants c1 c2))
  | t -> t
...
let simplify_comp i si op j sj =
  match op, (si, sj) with
    ...
    | (Eq | Neq) , _ ->
      let ti = Term.Elem (i, si) in
      let tj = Term.Elem (j, sj) in 
      if Term.compare ti tj < 0 then Atom.Comp (tj, op, ti)
      else Atom.Comp (ti, op, tj)
    | _ -> 
      Atom.Comp (Term.Elem (i, si), op, Term.Elem (j, sj))
	
let rec simplification np a =
  let a = redondant_or_false (SAtom.remove a np) a in
  match a with
    | Atom.True | Atom.False -> a 
    | Atom.Comp (Term.Elem (i, si), op , Term.Elem (j, sj)) -> simplify_comp i si op j sj
    | Atom.Comp (Term.Arith (i, csi), op, (Term.Arith (j, csj)))
    ...
    | Atom.Comp (Term.Arith (i, csi), op, (Term.Arith (j, csj))) ->
      ...
      else Atom.Comp (i, op, (Term.Arith (j, cs)))
    (* | Atom.Comp (Const cx, op, Term.Arith (y, sy, cy)) -> *)
    (* 	Atom.Comp (Const (add_constants (mult_const (-1) cx) cx), op, *)
    (* 	      Term.Arith (y, sy , (add_constants (mult_const (-1) cx) cy))) *)
    (* | Atom.Comp ( Term.Arith (x, sx, cx), op, Const cy) -> *)
    (* 	Atom.Comp (Term.Arith (x, sx , (add_constants (mult_const (-1) cy) cx)), op, *)
    (* 	      Const (add_constants (mult_const (-1) cy) cy)) *)
    | Atom.Comp (Term.Arith (x, cx), op, Term.Const cy) ->
      let mcx = mult_const (-1) cx in
      Atom.Comp (x, op, Term.Const (add_constants cy mcx))
    | Atom.Comp (Term.Const cx, op, Term.Arith (y, cy)) ->
      let mcy = mult_const (-1) cy in
      Atom.Comp (Term.Const (add_constants cx mcy), op, y)
        
    | Atom.Comp (x, op, Term.Arith (y, cy)) when Term.compare x y = 0 ->
	...
	if MConst.is_empty my then MConst.add c (i/(abs i)) my else cy in 
      Atom.Comp (Term.Const cx, op, Term.Const cy)
    | Atom.Comp (Term.Arith (y, cy), op, x) when Term.compare x y = 0 ->
      let cx = add_constants (mult_const (-1) cy) cy in
      let c, i = MConst.choose cy in
      let my = MConst.remove c cy in
      let cy = 
	if MConst.is_empty my then MConst.add c (i/(abs i)) my else cy in 
      Atom.Comp (Term.Const cy, op, Term.Const cx)
    | Atom.Comp (Term.Const c1, (Eq | Le), Term.Const c2) when compare_constants c1 c2 = 0 ->
      Atom.True 
    | Atom.Comp (Term.Const c1, Le, Term.Const c2) ->
      begin
	match MConst.is_num c1, MConst.is_num c2 with
	  | Some n1, Some n2 -> if Num.le_num n1 n2 then Atom.True else Atom.False
	  | _ -> a
      end
    | Atom.Comp (Term.Const c1, Lt, Term.Const c2) ->
      begin
	match MConst.is_num c1, MConst.is_num c2 with
	  | Some n1, Some n2 -> if Num.lt_num n1 n2 then Atom.True else Atom.False
	  | _ -> a
      end
    | Atom.Comp (Term.Const _ as c, Eq, y) -> Atom.Comp (y, Eq, c)
...
let inconsistent_list l = 
  let rec check values eqs neqs les lts ges gts = function
    ...
    | Atom.Comp (t1, Eq, (Term.Elem (x, s) as t2)) :: l 
    | Atom.Comp ((Term.Elem (x, s) as t2), Eq, t1) :: l ->
      (match s with
	| Var | Constr ->
...
let inconsistent_aux ((values, eqs, neqs, les, lts, ges, gts) as acc) = function
  ...
  | Atom.Comp (t1, Eq, (Term.Elem (x, s) as t2)) 
  | Atom.Comp ((Term.Elem (x, s) as t2), Eq, t1) ->
    (match s with
      | Var | Constr ->
	if assoc_neq t1 values t2 
	  || assoc_eq t1 neqs t2 || assoc_eq t2 neqs t1 
...
let tick_pos sa = 
  ...
  SAtom.iter
    (fun a -> match a with
      | Atom.Comp(Term.Const c,Lt, Term.Const m) when const_nul c -> 
	begin
	  try
	    let n = ref None in
	    MConst.iter 
  ...
  match e with
	...
	try
	  ...
	  if c > 0 then 
	    ...
	    in
	    simplification SAtom.empty (Atom.Comp (Term.Const m, Lt, x))
	...
	try
	  ...
	  if c > 0 then 
	    let m = MConst.remove tick m in
	    let e = 
	      if MConst.is_empty m then v else Term.Arith(v, m)
	    in
...
let remove_tick_atom sa (tick, at) = 
  ...
  let remove a sa = 
    let a = match a with
      | Atom.Comp ((Term.Const _ | Term.Arith (_, _) as e), (Le|Lt|Eq as op), x)
      | Atom.Comp (x, (Eq as op), (Term.Const _ | Term.Arith (_, _) as e))  ->
...
let rec term_globs t acc = match t with
  | Term.Elem (a, Glob) | Term.Access (a, _) -> Term.Set.add t acc
  | Term.Arith (x, _) -> term_globs x acc
  | _ -> acc
