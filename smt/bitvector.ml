...
module Make(X : ALIEN) = struct
  ...
  let compare x y = compare (*(embed x) (embed y)*) x y 
  ...
  let solve u t = 
    if Options.debug_bitv then eprintf "[Bitv] %a = %a@." X.print u X.print t;
    let u , t =    
      match X.extract u , X.extract t with 
	  Some u , Some t -> u,t 
	| None , Some t -> [{bv=Other(Alien u); sz=size_of u}] , t
	| Some u , None -> u , [{bv=Other(Alien t); sz=size_of t}]
	| _ -> assert false
    in
    try
