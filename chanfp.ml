...
let compat_evts (ed1, vals1) (ed2, vals2) =
  same_dir ed1 ed2 && (same_chan ed1 ed2 || no_chan ed1) &&
    (same_peer ed1 ed2 || no_peer ed1) &&
    (vals1 = [] (*&& vals2 = []*) ||
     vals1 <> [] && vals2 <> [] &&
    (* (vals1 = [] || vals2 = [] || *)
...
let sync_agree cs ef pf et pt relf relt =
  HMap.for_all (fun ef0 et0 ->
    if Chanrel.Rel.mem_eq ef0 ef relf then Chanrel.Rel.mem_eq et0 et relt
    else if Chanrel.Rel.mem_eq ef ef0 relf then Chanrel.Rel.mem_eq et et0 relt
    else true
  ) cs
  
let rel_agree cs ef pf et pt relf relt =
  HMap.for_all (fun ef0 et0 ->
    if Chanrel.Rel.mem_lt ef0 ef relf then Chanrel.Rel.mem_lt et0 et relt
    else if Chanrel.Rel.mem_lt ef ef0 relf then Chanrel.Rel.mem_lt et et0 relt
    else true
  ) cs
...
let make_substs esf relf est relt =
  ...
  let rec aux csl cs esf est =
    try
      let ef, (((df, pf, qf, cf) as edf, valf) as evtf) = HMap.choose esf in
      (* Format.fprintf Format.std_formatter "From %a\n" H.print ef; *)
      let esf = HMap.remove ef esf in
      let csl = HMap.fold (
        fun et (((dt, pt, qt, ct) as edt, valt) as evtt) csl ->
        (* Format.fprintf Format.std_formatter "Try %a\n" H.print et; *)
        if valf = [] && valt = []
           && H.equal pf pt
           && is_recv edf && is_recv edt
