...
let markov_entropy_detailed glob tsys all_procs trans steps matr =
  ...
  while  (!taken <= steps) && !running do
    try
      ...
      curr_env := check_reqs (fst proposal.tr_reqs) env sigma proposal.tr_name;
      let trargs = List.map (fun x -> Variable.subst sigma x) proposal_tr_args in

      let ureqs = List.map (fun (a,b,c) -> a,b) proposal.tr_ureq in 
...
let interpret_bfs original_env transitions all_procs all_unsafes =
  try
    ...
    Format.eprintf "ORIGINAL: %a@." print_interpret_env original_env;
    ...
    Format.printf "stuff in pool: %d@." !pool_size;
