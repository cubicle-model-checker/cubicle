...
let rec draw_dfs depth node turtle =
  ...
  if hspace_dist_sqr turtle <=  rlimit_sqr  then begin
    ...
    if n > 0 then begin
      let distance = step_from (if depth = 0 then max 3 n else 2 * max 3 n) in
      let angle = (if depth = 0 then 2. else 1.) *. pi /. (float_of_int n) in
      let turtle = 
...
let draw_node_and_edges root turtle = 
...
let draw_graph root turtle =
  ...
  (* if (G.V.label root).successors_visible then draw_node_and_edges root turtle *)
  (* else draw_node root turtle   *)
