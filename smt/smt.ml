...
module Formula = struct
  ...
  let rec print_ast fmt phi = 
    match phi with
      | Lit a -> Literal.LT.print fmt a
      | Comb (Not, [f]) -> 
	fprintf fmt "Comb(Not, %a)" print_ast f
      | Comb (And, l) -> fprintf fmt "Comb (And, [%a])" (print_ast_list ", ") l
      | Comb (Or, l) ->  fprintf fmt "Comb (Or, [%a])" (print_ast_list ", ") l
      | Comb (Imp, [f1; f2]) -> 
	fprintf fmt "Comb (Imp, %a, %a)" print_ast f1 print_ast f2
      | _ -> assert false
  and print_ast_list sep fmt = function
    | [] -> ()
    | [f] -> print_ast fmt f
    | f::l -> fprintf fmt "%a %s %a" print_ast f sep (print_ast_list sep) l
  ...
  let make_cnf f =
    eprintf "[smt] sform f %a@." print_ast f;
    let f = sform f in
    eprintf "[smt] make cnf %a@." print_ast f;
    let sfnc = cnf f in
    eprintf "[smt] cnf made@.";
    init [] sfnc
